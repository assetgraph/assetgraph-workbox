...
module.exports = async function addPrecacheServiceWorker(
  assetGraph,
  { query, single = false, minify = false, configPath } = {}
) {
  ...
  async function generateAndAttachPrecacheServiceWorkerAsync(
    ...
    const baseUrl = commonPathPrefix(
      entryPointAssetPaths.map(pathModule.dirname)
    );
